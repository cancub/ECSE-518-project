...
// 	srand(time(NULL));
// 	int num_rows = 9990;
// 	int * rows = (int*)calloc(num_rows+1,sizeof(int));
// 	// int * cols = (int*)calloc(3,sizeof(int));

// 	struct DubArray vector =  initialize_vector(n,rand() % 300);


// 	struct DubArray matrix = initialize_vector(n*n,0);
...
// 	{
// 		matrix.array[i] = ((double)(rand() % 100))/((double)(rand() % 10 + 1));
// 		// printf("matrix[%d] = %5.3f\n",i,matrix.array[i] );
// 	}

// 	//multiply with matrix one and check the time
// 	printf("starting first, non sparse\n");
// 	clock_t start = clock(), diff;
// 	alphaATtimesx(2.3,&matrix,&vector);
// 	diff = clock() - start;
// 	int msec = diff*1000/CLOCKS_PER_SEC;
// 	printf("Time to converge = %5.3f s\n", (double)(msec/1000)+((double)(msec%1000))/1000 );


// 	printf("changing rows\n");
// 	zero_rows(&matrix,rows);
// 	printf("changing cols\n");
// 	zero_cols(&matrix,rows);
// 	// print_DubMatrix(&matrix);

// 	//multiply 
// 	printf("starting second, sparse\n");
// 	start = clock();
// 	alphaATtimesx(2.3,&matrix,&vector);
// 	diff = clock() - start;
